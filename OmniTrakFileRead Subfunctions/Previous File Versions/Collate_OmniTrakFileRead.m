function Collate_OmniTrakFileRead(progpath, subpath)

%**************************************************************************
%This function is now deprecated. Use Deploy_OmniTrakFileRead instead.
%**************************************************************************

start_script = fullfile(subpath, 'OmniTrakFileRead_Beta.m');                %Set the expected name of the initialization script.
collated_filename = 'OmniTrakFileRead.m';                                   %Create the collated filename.
...
if exist(fullfile(progpath,'OmniTrakFileRead.m'),'file')                    %If the OmniTrakFileRead function already exists...
    backup_dir = fullfile(progpath,'Previous File Versions');               %Set the expected name of the backup directory.
    if ~exist(backup_dir,'dir')                                             %If the backup directory doesn't exist yet...
        mkdir(backup_dir);                                                  %Create it.
    end
    info = dir(fullfile(progpath,'OmniTrakFileRead.m'));                    %Grab the existing file information.
    time = datestr(info.datenum,'_yyyymmdd_HHMMSS');                        %Convert the last modified time to a string.
    filename = fullfile(backup_dir,...
        [collated_filename(1:end-2) time '.m']);                            %Add a timestamp to the filename.
    copyfile(fullfile(progpath,'OmniTrakFileRead.m'),...
        filename,'f');                                                      %Copy the previous version to a timestamped backup.
end
...
while numfuns ~= length(depfuns)                                            %Loop until no more dependencies are found.    
    ...
    for f = 1:length(depfuns)                                               %Step through each subfunction.        
        if strncmpi(progpath,depfuns{f},length(progpath))                   %If the subfunction is in the current directory...
...
clc;                                                                        %Clear the command line.
main_fid = fopen(fullfile(progpath,'OmniTrakFileRead.m'),'wt');             %Open a new *.m file to write the new script to.
fprintf(main_fid,'%s\n\n',...
...
fprintf(1,'%%Collated: %s\n\n',datestr(now,'mm/dd/yyyy, HH:MM:SS'));        %Print the compile time to the command line.
ln_num = 4;                                                                 %Create a variable to count the lines across the entire collated m-file.
for f = 1:length(depfuns)                                                   %Step through each included function.
    if strncmpi(progpath,depfuns{f},length(progpath))                       %If the subfunction is in the current directory...
        ...
        [path, filename, ~] = fileparts(depfuns{f});                        %Grab just the filename of the function.
        backup_dir = fullfile(path,'Previous File Versions');               %Set the expected name of the backup directory.
        if ~exist(backup_dir,'dir')                                         %If the backup directory doesn't exist yet...
            mkdir(backup_dir);                                              %Create it.
        end
