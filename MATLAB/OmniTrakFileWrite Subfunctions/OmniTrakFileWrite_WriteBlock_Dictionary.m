...
file_writer{'SESSION_PARAMS_JSON'} = ...
    @(varargin)OmniTrakFileWrite_WriteBlock_Struct_as_JSON(fid,...
...
% TTL_PULSETRAIN (0x0800): Timestamped event for a single TTL pulse output,
% with channel number, voltage, pulse duration, inter-pulse period, and 
% number of pulses.
file_writer{'TTL_PULSETRAIN'} = ...
    @(src, chan, varargin)...
    OmniTrakFileWrite_WriteBlock_TTL_PULSETRAIN(fid,...
    ofbc('TTL_PULSETRAIN'),...
    src, chan, varargin{:});

% TTL_PULSETRAIN_ABORT (0x0801): Timestamped event for when a TTL 
% pulsetrain is canceled before completion.
file_writer{'TTL_PULSETRAIN_ABORT'} = ...
    @(src, chan, varargin)...
    OmniTrakFileWrite_WriteBlock_TTL_PULSETRAIN_ABORT(fid,...
    ofbc('TTL_PULSETRAIN_ABORT'),...
    src, chan, varargin{:});
